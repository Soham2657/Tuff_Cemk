...
const Clubs = () => {
  ...
  return (
    <div className="flex-1 w-full pb-20 sm:pb-24">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-3 sm:gap-4">
        <div>
          <h2 className="font-h1 text-[30px] sm:text-[40px] font-bold text-on-surface tracking-tight mb-1">Campus Clubs</h2>
          <p className="font-body-lg text-[15px] sm:text-[18px] text-on-surface-variant">Discover communities, join discussions, and participate in events.</p>
        </div>
      ...
      {isAdmin && (
        <form onSubmit={handleCreateClub} className="mb-8 rounded-[20px] border border-outline-variant/30 bg-surface-container-lowest p-4 sm:p-6 grid gap-4 md:grid-cols-2">
        ...
        {loading ? (
          ...
          return (
            <article key={club._id} className="bg-linear-to-br from-surface-container-lowest to-surface-container-low rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-outline-variant/40 overflow-hidden flex flex-col hover:shadow-[0_16px_50px_rgba(0,0,0,0.12)] hover:border-secondary/30 transition-all duration-300 group">
              <div className="h-28 sm:h-32 w-full bg-surface-container relative">
                <img 
                  alt={club.name} 
              ...
              <div className="p-4 flex-1 flex flex-col gap-4">
                <h3 className="font-h3 text-[20px] sm:text-[24px] font-bold text-on-surface mb-0 group-hover:text-primary transition-colors">{club.name}</h3>
                <p className="font-body-md text-[15px] sm:text-[16px] text-on-surface-variant line-clamp-2">{club.description}</p>
                <div className="flex items-center gap-2 mt-auto pt-4 border-t border-outline-variant/50">
                  ...
                  <div className="ml-auto w-full sm:w-auto">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                      <Link to={`/clubs/${club._id}`} className="w-full sm:w-auto text-center px-4 py-2 rounded-lg border border-outline-variant/20 text-on-surface-variant hover:bg-surface-container-low transition-colors">
                        View
                      </Link>
                      {isAdmin ? (
                        <button onClick={() => handleDeleteClub(club._id)} className="w-full sm:w-auto px-5 py-2 bg-error text-on-error font-label-bold text-[14px] font-semibold rounded-xl shadow-sm">
                          Delete
                        </button>
                      ) : member ? (
                        ...
                        <button
                          ...
                          disabled={pending}
                          className="w-full sm:w-auto px-5 py-2 bg-primary text-on-primary font-label-bold text-[14px] font-semibold rounded-xl hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-70"
                        >
